...
class midi_processor(object):
    ...
    def tempoNormalize(self, midi_data):
        ...
        for item in ts:
            ...
            new_time = 0
            idx = 0
            while time_stamps[idx+1] < time:
                ratio = tempi[idx] / int(round(np.mean(tempi)))
                new_time += (time_stamps[idxT+1] - time_stamps[idxT]) *  ratio
                idx += 1
            ratio = tempi[idx] / int(round(np.mean(tempi)))
            new_time += (time - time_stamps[idx]) * ratio
            new_ts = pyd.TimeSignature(numerator, denominator, new_time)
            new_time_signature_changes.append(new_ts)
        midiReGen.time_signature_changes = new_time_signature_changes
        #key signature
        new_key_signature_changes = []
        for item in ks:
            key = item.key_number
            time = item.time
            new_time = 0
            ...
            ratio = tempi[idx] / int(round(np.mean(tempi)))
            new_time += (time - time_stamps[idx]) * ratio
            new_ks = pyd.KeySignature(key, new_time)
            new_key_signature_changes.append(new_ks)
        midiReGen.key_signature_changes = new_key_signature_changes
        return midiReGen
